...
<script>
    ...
    export default {
        ...
        methods: {
            onSubmit() {
                if (this.$route.params.formType === "Edit") {
                    this.$store.dispatch("GroupService/editGroup", this.Group).then(res => {
                        if (res.status === 200) {
                            this.closeDialog();
                        }
                    });
                } else if (this.$route.params.formType === "Insert") {
                    this.Group.agents = JSON.stringify(this.Group.agents);
                    this.$store.dispatch("GroupService/addGroup", this.Group).then(res => {
                        if (res.status === 201) {
                            this.closeDialog();
                        }
                    });
        ...
        created() {
            ...
            this.$store.dispatch("MemberService/loadMembers").then(() => {
                this.Members = this.$store.getters[
